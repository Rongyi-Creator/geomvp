...
const BUSINESS = {
  ...
  description:
    "Virum Akupunktur tilbyder en unik holistisk service til kunder gennem evaluering og skabelse af passende behandlingsforløb baseret på Traditionel Kinesisk Medicin.",
...
const FAQ_ITEMS = [
  {
    ...
    answer:
      "Akupunktur er en flere tusinde år gammel behandlingsform, der har sin oprindelse i Traditionel Kinesisk Medicin.",
  },
  {
    question: "Hvordan foregår en akupunkturbehandling?",
    answer:
      "Det 1. besøg hos Virum Akupunktur starter altid med en indledende samtale, hvor akupunktøren danner sig et indtryk af patienten, og den første behandling starter med en grundig undersøgelse, hvor diagnosen stilles ifølge TCM's teori.",
  },
  {
    question: "Hvad koster første behandling?",
    answer:
      "Specialtilbud for første prøvebehandling ca. 75 min. med gratis undersøgelse og behandling koster 450 kr.",
  },
  {
    question: "Hvor er klinikken placeret, og er der parkering?",
    answer:
      "Virum Akupunktur ligger på Dalstrøget 78, 4. sal, 2870 Dyssegård (tag elevator til 4. sal). Der er ubegrænset parkering ved Dalstrøget parkeringsplads. Bus linjerne 164, 4A og 6A kører i nærheden.",
  },
  {
...
const PAGES_META: Record<string, PageMeta> = {
...
# Allow: retrieval/citation bots (GEO targets)
# Disallow: training-only crawlers (no citation value)
...
Sitemap: https://virumakupunktur.dk/sitemap.xml
`;
...
function serveSitemap(): Response {
  ...
  const urls = paths
    .map(
      (p) =>
        `  <url><loc>https://virumakupunktur.dk${p === "/" ? "/" : p}</loc></url>`,
    )
    .join("\n");
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const canonicalHref = `https://virumakupunktur.dk${canonicalPath === "/" ? "/" : canonicalPath}`;
