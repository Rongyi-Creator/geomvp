/**
 * Edge Pipeline Step 3 (Vercel): Generate Vercel Edge Function from extracted GEO data
 * Usage: tsx scripts/edge/03-generate-vercel.ts <client-name> [--origin-host https://www.example.com]
 *
 * Reads geo-data/ (business.json, faq.json, services.json, pages-meta.json)
 * and produces a complete Vercel Edge Function proxy at clients/<client>/vercel-edge/
...
if (!clientName) {
  console.error('Usage: tsx scripts/edge/03-generate-vercel.ts <client-name> [--origin-host https://www.example.com]');
  process.exit(1);
}

const originHostFlag = args.indexOf('--origin-host');
let customOriginHost: string | null = originHostFlag >= 0 ? args[originHostFlag + 1] : null;

const dirs = getCloneDir(clientName);
const geoDir = dirs.geoData;
const vercelDir = resolve(ROOT, 'clients', clientName, 'vercel-edge');
const apiDir = resolve(vercelDir, 'api');
const libDir = resolve(vercelDir, 'lib');

// ── Verify inputs ───────────────────────────────────────────────────────────

const requiredFiles = ['business.json', 'faq.json', 'services.json', 'pages-meta.json'];
for (const f of requiredFiles) {
  const p = resolve(geoDir, f);
  if (!existsSync(p)) {
    console.error(`\n❌ Missing ${f} in ${geoDir}`);
...
console.log(`\n🔧 Generating Vercel Edge Function for "${clientName}"`);
...
// Derive host URLs
const websiteUrl = (business.website as string).replace(/\/$/, '');
const websiteUrlObj = new URL(websiteUrl);
const rootDomain = websiteUrlObj.hostname.replace(/^www\./, '');
const canonicalHost = `https://${rootDomain}`;
const originHost = customOriginHost ?? `https://www.${rootDomain}`;

// Short client id for dashboard reporting (first segment before dash)
const dashboardClient = clientName.split('-')[0];

console.log(`   Canonical: ${canonicalHost}`);
console.log(`   Origin:    ${originHost}`);
if (customOriginHost) console.log(`   (using custom origin host from --origin-host flag)`);

// ── Build BUSINESS schema object ────────────────────────────────────────────
...
function buildServicesRecord(): string {
  ...
  for (const svc of services) {
    const pageMeta = pagesMeta.find(
      (p) => p.slug === svc.slug || p.path === `/${svc.slug}/`,
    );
    const path = pageMeta?.path ?? `/${svc.slug}/`;
    entries.push(`  ${JSON.stringify(path)}: { name: ${JSON.stringify(svc.name)}, description: ${JSON.stringify(svc.description)} },`);
  }
  return entries.join('\n');
}
...
function buildPagesMeta(): string {
  ...
  for (const p of pagesMeta) {
    ...
    entries.push(
      `  ${JSON.stringify(p.path)}: { pageType: ${JSON.stringify(p.pageType)}, metaTitle: ${JSON.stringify(p.metaTitle)}, metaDescription: ${JSON.stringify(p.metaDescription)} },`,
    );
  }
  return entries.join('\n');
}

const geoDataTs = `export const ORIGIN_HOST = ${JSON.stringify(originHost)};
export const CANONICAL_HOST = ${JSON.stringify(canonicalHost)};

export const BUSINESS = ${JSON.stringify(businessSchema, null, 2)};

export const FAQ_ITEMS = ${JSON.stringify(faq.items || [], null, 2)};
...
const DASHBOARD_CLIENT = ${JSON.stringify(dashboardClient)};
...
writeFileSync(resolve(libDir, 'geo-data.ts'), geoDataTs);
console.log(`   ✅ lib/geo-data.ts (${(geoDataTs.length / 1024).toFixed(1)} KB)`);

writeFileSync(resolve(apiDir, 'proxy.ts'), proxyTs);
console.log(`   ✅ api/proxy.ts (${(proxyTs.length / 1024).toFixed(1)} KB)`);

const vercelJson = JSON.stringify({
  rewrites: [{ source: '/(.*)', destination: '/api/proxy' }],
}, null, 2);
writeFileSync(resolve(vercelDir, 'vercel.json'), vercelJson);
console.log(`   ✅ vercel.json`);

const workerName = `geo-edge-${clientName}-vercel`;
...
writeFileSync(resolve(vercelDir, 'package.json'), packageJson);
...
writeFileSync(resolve(vercelDir, 'tsconfig.json'), tsconfig);
console.log(`   ✅ tsconfig.json`);

console.log(`\n✅ Vercel Edge Function generated at: ${vercelDir}`);
console.log(`   ${pagesMeta.filter(p => !p.isEmpty).length} pages, ${services.length} services, ${(faq.items || []).length} FAQ items`);
console.log(`   Origin: ${originHost}  →  Canonical: ${canonicalHost}`);
console.log(`\nNext steps:`);
console.log(`  1. cd ${vercelDir} && vercel link   (first time only)`);
console.log(`  2. tsx scripts/edge/04-deploy-vercel.ts ${clientName}`);
